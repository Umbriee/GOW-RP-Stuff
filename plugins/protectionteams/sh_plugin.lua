...
ix.command.Add("SquadCreate", {
	...
	OnRun = function(self, client, index)
		...
		if (!index) then
			return client:RequestString("@cmdPTCreate", "@cmdCreatePTDesc", function(text) ix.command.Run(client, "SquadCreate", {text}) end, "")
		end
...
ix.command.Add("SquadReassign", {
	...
	OnRun = function(self, client, newIndex, oldIndex)
		...
		if (newIndex and oldIndex) then
			...
			return client:RequestString("@cmdPTReassign", "@cmdReassignPTDesc", function(text) ix.command.Run(client, "SquadReassign", {text, oldIndex}) end, "")
		end
	end
})
